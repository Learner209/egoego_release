...
from egoego.lafan1.utils import quat_inv, quat_mul, quat_normalize
...
def test(opt, device):
	...
	with torch.no_grad():
		...
		for take_uid in egoexo_poses.files:
			...
			egoexo_to_opengl_rot_mat = transforms.euler_angles_to_matrix(torch.tensor([0, 0, 1], dtype=torch.float32)[None],convention="XYZ")
			egoexo_to_opengl_quat = transforms.matrix_to_quaternion(egoexo_to_opengl_rot_mat)
			egoexo_to_opengl_quat = egoexo_to_opengl_quat.repeat(this_egoexo_quat.shape[0], 1)
			this_egoexo_quat = quat_mul(this_egoexo_quat, egoexo_to_opengl_quat)

			this_egoexo_pose = np.concatenate([this_egoexo_trans, this_egoexo_quat], axis=-1) # T x 7
